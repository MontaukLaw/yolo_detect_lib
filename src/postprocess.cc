...
static float deqnt_affine_to_f32(uint8_t qnt, uint32_t zp, float scale) { return ((float)qnt - (float)zp) * scale; }
...
int post_process(uint8_t *input0, uint8_t *input1, uint8_t *input2, int model_in_h, int model_in_w,
                 ...
                 std::vector<uint32_t> &qnt_zps, std::vector<float> &qnt_scales, detect_results_t *group)
{
    static int init = -1;
    if (init == -1)
    {
    ...
    memset(group, 0, sizeof(detect_results_t));
